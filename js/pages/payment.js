...
document.querySelector("#app").innerHTML = `
  ...
  <section class="section top">
    <div class="payment-layout">
      <div class="form-card"><p class="eyebrow">Advance Payment</p><h2>Reservation summary</h2><div class="summary-list"><div><span>Service</span><strong>${service.name}</strong></div><div><span>Barber</span><strong>${byId(barbers, booking.barberId).name}</strong></div><div><span>Date and time</span><strong>${booking.date} ${booking.time}</strong></div><div><span>Cut price</span><strong>${peso(service.price)}</strong></div><div><span>Online booking fee</span><strong>${peso(bookingFee)}</strong></div><div><span>Total advance payment</span><strong>${peso(total)}</strong></div></div><div class="payment-methods">${["GCash", "Maya"].map((m) => `<button class="method ${state.paymentMethod === m ? "active" : ""}" type="button" data-payment="${m}">${m}</button>`).join("")}</div><label class="upload-box">Upload proof of payment<input type="file"></label><button class="button primary full" type="button" data-confirm-payment>Submit for verification</button></div>
...
document.querySelector("[data-confirm-payment]").addEventListener("click", async () => {
  try {
    const payload = await api("/appointments", {
      method: "POST",
      body: JSON.stringify({ ...booking, bookingFee, total, source: "online", paymentMethod: state.paymentMethod })
    });
    toast(`Booking created. Queue #${String(payload.appointment.queueNumber).padStart(2, "0")}.`);
  } catch (error) {
    const appointmentNumbers = state.appointments.map((item) => Number(item.id || 0)).filter(Number.isFinite);
    const nextId = Math.max(0, ...appointmentNumbers) + 1;
    state.appointments.push({ id: nextId, customer: state.user.name || booking.email, barberId: booking.barberId, serviceId: booking.serviceId, date: booking.date, time: booking.time || "Next available", source: "online", bookingFee, total, status: "Pending verification", paid: true, paymentMethod: state.paymentMethod });
    save();
